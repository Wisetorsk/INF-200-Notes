...
def compute_dose(temp, rh, n = 730):
	...
	for a in range(n):
		...
		if temp[a] > 29:
			...
			Dt = np.exp(2.0*np.log(temp[a]/20.0))
...
def plot_mould(mnorth, msouth):
	...
	"""
	plt.subplot(2,1,1)
	plt.plot(val1, val2, label= 'Kurvelabel')
	plt.subplot(2,1,2)
	plt.plot(val3, val4, label= 'Kurvelabel 2')
	plt.grid()
	plt.xlabel('Label')
	plt.ylabel('Label')
